...
class AnimationScriptParser(ABC):
    def __init__(self, script_path: str):
        ...
        self._start_scene_pattern: str = r'<<([^\/].*)>>'
        self._end_scene_pattern:   str = r'<<\/(.*)>>'
    ...
    def _get_file_contents(self) -> str:
        ...
        '''
        contents = None
        with open(self._script_path, encoding='UTF-8') as read_file:
    ...
    def get_scene_text(self, start_scene_line: str, file_contents: str) -> str:
        ...
        scene_text_match: re.Match[str] | None = re.search(
            pattern=full_scene_pattern, string=file_contents, flags=re.DOTALL,
        )
        if scene_text_match is None:
            raise Exception(
                f'Unable to find scene text for scene starting with {start_scene_line}',
            )

        return scene_text_match.group(1).strip()

    def get_section_animation_chunks(self, section: str) -> str:
        animation_chunks: dict[str, list] = {
            'animation_lines': [],
            'tags': [],
        }
        if self._is_explicit_animation_section(section):
            animation_chunks['animation_lines'] = [
                line for i,
                line in enumerate(section.splitlines()) if i != 0
            ]
            animation_chunks['tags'] = self._get_animation_section_tags(
                section,
            )
        else:
            animation_chunks['animation_lines'] = [
    ...
    def _get_animation_section_name(self, section: str) -> str:
        ...
        name_with_tags = re.search(
            pattern=self._section_pattern, string=first_line,
        ).group(1).strip()
    ...
    def _get_animation_section_tags(self, section: str) -> list[str]:
        ...
        if len(name_with_tags.split(self._name_tag_separator)) == 1:
            return []

        tags_str_form = name_with_tags.split(
            self._name_tag_separator,
        )[1].split(',')

        tags_enum_form = []
        for tag_str in tags_str_form:
            if tag_str == 'code_timing':
                tags_enum_form.append(Tag.CODE_TIMING)
            else:
