...
class AlignmentFileFixer:
    def __init__(self, parsings_path, timings_path):
        ...
        self.aligned_script_path = os.path.join(
            os.path.dirname(self.parsings_path), 'aligned_script.txt',
        )
    ...
    def combine_parsings_and_timings(self):
        words = []
        with open(self.parsings_path) as file:
            content = file.read()
        ...
        with open(self.timings_path) as file:
