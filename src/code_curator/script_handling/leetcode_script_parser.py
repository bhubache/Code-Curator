            ...
            for section_name, section_info in section_map.items():
                ...
                if len(animation_lines) == 1:
                    ...
                    composite_sections.append(
                        AnimationLeaf(
                            unique_id=section_name, text=animation_lines[
                                0
                            ], is_wait_animation=is_wait_animation, tags=tags,
                        ),
                    )
                else:
                    # Composite
                    composite_sections.append(
                        CompositeAnimationScript(
                            ...
                            children=[
                                AnimationLeaf(
                                    unique_id=f'{section_name}_{i}', text=line, is_wait_animation=False, tags=tags,
                                ) for i, line in enumerate(animation_lines)
                            ],
                        ),
                    )
