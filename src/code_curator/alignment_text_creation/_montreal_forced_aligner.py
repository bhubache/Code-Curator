...
class MontrealForcedAligner:
    ...
    def _verify_input_and_output_dirs(self) -> None:
        if not self.input_dir_path.exists() or not self.output_dir_path.exists():
            raise FileNotFoundError(f'The following two directories are required for montreal forced aligned: {self.input_dir_path} and {self.output_dir_path}')

    def _verify_input_dir_has_two_files(self) -> None:
        if (num_files := len(list(self.input_dir_path.iterdir()))) != 2:
            raise ValueError(f'{self.input_dir_path} must only have 2 files: a text file and an audio file... {num_files} found.')
