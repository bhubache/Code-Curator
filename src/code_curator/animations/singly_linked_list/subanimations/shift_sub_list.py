...
class ShiftSubList(LeafSubanimation):
    ...
    def begin(self):
        ...
        self._group_to_move_to = VGroup(
            *[
                node for i, node in enumerate(
                self._sll_post_subanimation_group,
                ) if i > self._index
    ...
    def interpolate(self, alpha: float):
        ...
        self._sub_list_to_shift.move_to(
            self._sub_list_original_start + (
            (self._group_to_move_to.get_center() - self._sub_list_to_shift.get_center()) * smooth(alpha)
            ),
        )
