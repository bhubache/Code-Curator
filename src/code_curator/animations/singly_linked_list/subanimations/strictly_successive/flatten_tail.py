...
class SuccessiveFlattenTail(LeafSubanimation):
    def __init__(self, sll, index: int, added_node: SLLNode):
        ...
        self._sub_list_to_shift: VGroup = VGroup(
            *[
                node for i, node in enumerate(self._sll)
                if i > self._index
    ...
    def begin(self):
        ...
        self._total_node_shift_up_distance = abs((
            self._added_node.get_container_top(
            ) - self._final_sll[self._index].get_container_top()
        )[1])
        ...
        # self._sub_list_total_distance = abs(
        # self._sub_list_to_shift[0].get_left() - self._final_sll_copy[self._index + 1].get_left()
        # )

    def interpolate(self, alpha: float):
        self._added_node.container.move_to(
            self._added_node_original_start + (
                [0, self._total_node_shift_up_distance * smooth(alpha), 0]
            ),
        )

        # self._sub_list_to_shift.move_to(
        # self._sub_list_original_start + (self._sub_list_total_distance * smooth(alpha))
        # )
