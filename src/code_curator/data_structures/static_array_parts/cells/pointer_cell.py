...
class PointerCell(Cell):
    ...
    def add_pointer_to_front(self, pointer):
        if not isinstance(pointer, Pointer):
            raise ValueError(
                'pointer must be of type Pointer: not {type(pointer)}',
            )
    ...
    def _distribute_elements(self):
        ...
        for i in range(num_pointers):
            if not hasattr(self._pointers[i], 'new'):
                animations.append(
                    self._pointers[i].animate.move_to(
                    starting_location + [space * ((i * 2) + 1), 0, 0],
                    ),
                )
            else:
                self._pointers[i].move_to(
                    starting_location + [space * ((i * 2) + 1), 0, 0],
                )
        ...
        for i in range(num_barriers):
            ...
            if not hasattr(self._barriers[i], 'new'):
                animations.append(
                    self._barriers[i].animate.put_start_and_end_on(
                    barrier_start, barrier_end,
                    ),
                )
            else:
                self._barriers[i].put_start_and_end_on(
                    barrier_start, barrier_end,
                )
    ...
    def _add_pointer_internal(self, name, index):
        ...
        if len(self._pointers) > 1:
            self._barriers.insert(
                index, PointerBarrier(
                pointer_name=pointer.get_value(),
                ),
            )
    ...
    def _get_barrier(self, pointer):
        for i, barrier in enumerate(self._barriers):
            if barrier.is_linked_to_pointer(pointer):
                return barrier
        raise Exception(
            f'Can\'t find barrier associated with pointer {pointer}',
        )

    def contains_pointer_name(self, name):
        find_pointer = Pointer(name)
        for pointer in self._pointers:
            if pointer == find_pointer:
                return True
        return False
