...
class ElementRow(Row):
    ...
    def _create_init_values(self) -> Iterable[ElementCell]:
        ...
        values = []
        if self._underflow:
            values.append(ElementCell('-', -1))
        for i in range(self._length):
            values.append(ElementCell(0, i))
        if self._overflow:
            values.append(ElementCell('-', self._length))
        return values
    ...
    def copy_value_from_index_to_index(self, from_index: int, to_index: int) -> Iterable[Animation]:
        ...
        self._cells[to_index].set_value(
            self._cells[from_index].get_value().copy(),
        )

        self._cells[to_index]._value.move_to(self._cells[from_index])

        animations.append(
            self._cells[to_index]._value.animate.move_to(self._cells[to_index]),
        )
        return animations
    ...
    def compare(self, *indices: Iterable[int], fn=None, true_color=None, false_color=None) -> Iterable[Animation]:
        ...
        cells_to_compare = [
            cell.get_value()
            for i, cell in self._cells.items() if i in indices
        ]
