...
class StaticMatrix(VMobject):
    ...
    def move_pointer(self, name: str, num_rows: int = 0, num_columns: int = 0) -> Iterable[Animation]:
        ...
        pointer_to_move = from_row.pointers._cells[from_column_index].get_pointer(
            name,
        )
        ...
        if num_rows > 0:
            animations += to_row.add_pointer_to_front(
                to_column_index, name, arg_pointer=pointer_to_move,
            )
        else:
            animations += to_row.add_pointer_to_back(
                to_column_index, name, arg_pointer=pointer_to_move,
            )

        to_row.pointers._remove_animation(FadeIn, pointer_to_move, animations)

        from_cell = from_row.pointers._cells[
            from_row.pointers._index_of_pointer(
            name,
            )
        ]
        animations += from_cell.remove_pointer(name)
        from_row.pointers._remove_animation(
            FadeOut, pointer_to_move, animations,
        )
        new_pointer_location = to_row.pointers._cells[to_column_index].get_pointer(
            name,
        ).get_center()
    ...
    def copy_value_from_index_to_index(self, from_row: int, from_column: int, to_row: int, to_column: int) -> Iterable[Animation]:
        ...
        animations = [
            *self._matrix[to_row].elements._cells[to_column].fade_value(),
        ]

        self._matrix[to_row].elements._cells[to_column].set_value(
            self._matrix[from_row].elements._cells[from_column].get_value().copy(),
        )

        self._matrix[to_row].elements._cells[to_column]._value.move_to(
            self._matrix[from_row].elements._cells[from_column],
        )

        animations.append(
            self._matrix[to_row].elements._cells[to_column]._value.animate.move_to(
            self._matrix[to_row].elements._cells[to_column],
            ),
        )
        return animations
    ...
    def swap(self, row_i: int, column_i: int, row_j: int, column_j: int) -> Iterable[Animation]:
        ...
        animations = self._move_element_from_index_to_index(
            row_j, column_j, row_i, column_i,
        )

        self._matrix[row_i].elements._cells[column_i].set_value(old_i_value)

        animations += self._move_element_from_index_to_index(
            row_i, column_i, row_j, column_j,
        )

        self._matrix[row_i].elements._cells[column_i].set_value(
            old_j_value.copy(),
        )
    ...
    def compare(self, *coords: Iterable[tuple[int]], fn=None, true_color=None, false_color=None) -> Iterable[Animation]:
        if true_color is None:
            true_color = Color(hue=7/20, saturation=1, luminance=0.5)
        if false_color is None:
            false_color = Color(hue=0, saturation=1, luminance=0.5)
        ...
        for i, arr in enumerate(self._matrix):
            ...
            for coord in coords:
                if coord[0] == i:
                    values_to_compare.append(
                        arr.elements._cells[coord[1]].get_value().get_value(),
                    )
        ...
        for row, column in coords:
            ...
            indicate_square = Rectangle(
                width=cell.width, height=cell.height, stroke_width=0,
                color=config.background_color, z_index=-1,
            ).move_to(cell).set_fill(opacity=0.5)
            animations.append(
                Indicate(indicate_square, color=color, scale_factor=1),
            )
    ...
    def _move_element_from_index_to_index(self, from_row: int, from_column: int, to_row: int, to_column: int) -> Iterable[Animation]:
        animations = []
        self._matrix[to_row].elements._cells[to_column].set_value(
            self._matrix[from_row].elements._cells[from_column].get_value(),
        )
...
class StaticMatrixBuilder:
    ...
    def __init__(self, num_rows=4, num_columns=4, descriptors=False, underflow=False, overflow=False, pointers=False):
        self._instance = StaticMatrix(
            num_rows=num_rows, num_columns=num_columns,
        )
        self._num_rows = num_rows
        self._num_columns = num_columns
        # This will be used for static matrix specific descriptors
    ...
    def add_elements(self) -> StaticMatrixBuilder:
        ...
        for i in range(self._num_rows):
            base_arr_builder = StaticArrayBuilder(
                length=self._num_columns, descriptors=self._has_descriptors, underflow=self._underflow, overflow=self._overflow,
            ).add_elements()
