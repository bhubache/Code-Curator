...
class BaseProblemAnalysis(BaseScene):
    def __init__(self, constraints, explanations, problem_dir, aligned_animation_scene):
        ...
        self._constraints_analysis_table = ProblemText.create_constraints_table(
            self._constraints_list, self._explanations,
        )
    ...
    def add_overriding_animation(self, method):
        self._aligned_animation_scene.add_animation(
            unique_id=method.__name__, func=method, animation=method(), is_overriding_animation=True,
        )
    ...
    def _init_constraints_animations(self):
        ...
        for index in range(1, len(self._explanations) + 1):
            if index == 0:
                continue
            constraints_dict[f'explanation_{index}_fade_in'] = self._present_single_problem_explanation(
                index=index,
            )
