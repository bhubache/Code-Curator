...
from manim import Rectangle, YELLOW, Code, LEFT, UP, DOWN, AnimationGroup, Succession
...
class CodeHighlighter(Rectangle):
    def __init__(self, code: Code, height: float = None, width: float = None, color = YELLOW, stroke_width = 0):
        self._custom_init_super(code=code, height=height, width=width, color=color, stroke_width=stroke_width)
    ...
    def move(self, num_lines: int) -> None:
        ...
        if new_line_num < 0 or new_line_num >= self.code.num_lines:
            raise IndexError(f'{new_line_num} is out of bounds for {self.code.num_lines}')
